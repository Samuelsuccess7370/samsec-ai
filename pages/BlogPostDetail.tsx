...
const BlogPostDetail = () => {
  // Manual params extraction from hash routing: #/blog/:id
    ...
    return (
      <div className="min-h-screen pt-24 pb-12 bg-background flex items-center justify-center">
        <Container className="text-center">
            ...
            <a href={`#${RoutePath.BLOG}`} className="text-secondary hover:text-white transition-colors">Return to Base</a>
        </Container>
      </div>
    );
  }
  ...
  return (
    <div className="pt-24 pb-12 min-h-screen bg-background">
      <Container className="max-w-4xl mx-auto">
        {/* Navigation */}
        <a href={`#${RoutePath.BLOG}`} className="inline-flex items-center text-gray-400 hover:text-secondary mb-8 transition-colors group">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {relatedPosts.map(rp => (
              <GlassCard key={rp.id} className="flex flex-col h-full group p-0" hoverEffect={true}>
                 <a href={`#/blog/${rp.id}`} className="flex flex-col h-full p-6">
